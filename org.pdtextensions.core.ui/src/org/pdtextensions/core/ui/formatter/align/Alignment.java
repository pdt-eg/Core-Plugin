 ...
 *******************************************************************************/
package org.pdtextensions.core.ui.formatter.align;
...
public class Alignment {
	...
	public static final int SPLIT_MASK = M_ONE_PER_LINE_SPLIT
			| M_NEXT_SHIFTED_SPLIT | M_COMPACT_SPLIT
	...
	public Alignment(int kind, int mode, int tieBreakRule, Scribe scribe,
			int fragmentCount, int sourceRestart, int continuationIndent) {
		...
		if ((mode & M_INDENT_ON_COLUMN) != 0) {
			...
			this.breakIndentationLevel = this.scribe
					.getNextIndentationLevel(currentColumn);
			...
			this.breakIndentationLevel = this.location.outputIndentationLevel
					+ indentSize;
		} else {
			this.breakIndentationLevel = this.location.outputIndentationLevel
					+ continuationIndent * indentSize;
		}
		this.shiftBreakIndentationLevel = this.breakIndentationLevel
				+ indentSize;
	...
	public void checkColumn() {
		if ((this.mode & M_MULTICOLUMN) != 0) {
			int currentIndentation = this.scribe
					.getNextIndentationLevel(this.scribe.column
							+ (this.scribe.needSpace ? 1 : 0));
			...
			if (this.needRedoColumnAlignment
					&& this.fragmentIndex == this.fragmentCount - 1) { // alignment too small
				...
				int relativeDepth = 0;
				Alignment targetAlignment = null;//XXX:this.scribe.memberAlignment;
				while (targetAlignment != null) {
					if (targetAlignment == this) {
						throw new AlignmentException(
								AlignmentException.ALIGN_TOO_SMALL,
								relativeDepth);
					}
	...
	public void performFragmentEffect() {
		...
		if (this.startingColumn < 0
				|| (fragmentIndentation + 1) < this.startingColumn) {
	...
	public String toString(StringBuffer buffer, int level) {
		...
		buffer.append("<kind: ") //$NON-NLS-1$
				.append(this.kind).append("> "); //$NON-NLS-1$
		...
		buffer.append("<depth=") //$NON-NLS-1$
				...
				.append(this.shiftBreakIndentationLevel).append(">\n"); //$NON-NLS-1$

		// Line to display the location
		buffer.append(indentation);
		buffer.append("<location=") //$NON-NLS-1$
				.append(this.location.toString()).append(">\n"); //$NON-NLS-1$
