...
public abstract class ModifyDialogTabPage implements IModifyDialogTabPage {
	...
	protected class ButtonPreference extends Preference {
		...
		public ButtonPreference(Composite composite, int numColumns,
				Map preferences, String key, String[] values, String text,
				int style) {
	...
	protected final class CheckboxPreference extends ButtonPreference {
		public CheckboxPreference(Composite composite, int numColumns,
				Map preferences, String key, String[] values, String text) {
			super(composite, numColumns, preferences, key, values, text,
					SWT.CHECK);
		}
	}
	...
	protected final class NumberPreference extends Preference {
		...
		public NumberPreference(Composite composite, int numColumns,
				Map preferences, String key, int minValue, int maxValue,
				String text) {
			super(preferences, key);
			...
			if (hasKey) {
				...
				try {
					...
					final String message = Messages
							.format(FormatterMessages.ModifyDialogTabPage_NumberPreference_error_invalid_key,
									getKey());
					PEXUIPlugin.log(new Status(IStatus.ERROR,
							PEXUIPlugin.PLUGIN_ID, IStatus.OK, message, e));
					s = ""; //$NON-NLS-1$
				}
	...
	protected final class StringPreference extends Preference {
		...
		public StringPreference(Composite composite, int numColumns,
				Map preferences, String key, String text,
		...
		private IStatus createErrorStatus(String errorText) {
			return new Status(IStatus.ERROR, PEXUIPlugin.PLUGIN_ID, 0,
					errorText, null);
		}
	...
	protected final static class DefaultFocusManager extends FocusAdapter {
		...
		private final static String PREF_LAST_FOCUS_INDEX = PEXUIPlugin.PLUGIN_ID
	...
	protected NumberPreference createNumberPref(Composite composite,
		...
		final NumberPreference pref = new NumberPreference(composite,
				numColumns, fWorkingValues, key, minValue, maxValue, name);
	...
	protected StringPreference createStringPref(Composite composite,
		...
		StringPreference pref = new StringPreference(composite, numColumns,
				fWorkingValues, key, name, inputValidator);
		fDefaultFocusManager.add(pref);
		pref.addObserver(fUpdater);
		return pref;
	}
	...
	protected CheckboxPreference createCheckboxPref(Composite composite,
		...
		final CheckboxPreference pref = new CheckboxPreference(composite,
				numColumns, fWorkingValues, key, values, name);
		fDefaultFocusManager.add(pref);
		pref.addObserver(fUpdater);
		return pref;
	}
