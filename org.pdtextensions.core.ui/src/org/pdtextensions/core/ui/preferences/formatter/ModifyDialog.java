...
public abstract class ModifyDialog extends StatusDialog implements
	...
	public ModifyDialog(Shell parentShell, Profile profile,
			ProfileManager profileManager, ProfileStore profileStore,
			boolean newProfile, String dialogPreferencesKey,
		...
		fKeyPreferredWidth = PEXUIPlugin.PLUGIN_ID + dialogPreferencesKey
				+ DS_KEY_PREFERRED_WIDTH;
		fKeyPreferredHight = PEXUIPlugin.PLUGIN_ID + dialogPreferencesKey
				+ DS_KEY_PREFERRED_HEIGHT;
		fKeyPreferredX = PEXUIPlugin.PLUGIN_ID + dialogPreferencesKey
				+ DS_KEY_PREFERRED_X;
		fKeyPreferredY = PEXUIPlugin.PLUGIN_ID + dialogPreferencesKey
				+ DS_KEY_PREFERRED_Y;
		fKeyLastFocus = PEXUIPlugin.PLUGIN_ID + dialogPreferencesKey
				+ DS_KEY_LAST_FOCUS;
		...
		fProfile = profile;
		setTitle(Messages.format(FormatterMessages.ModifyDialog_dialog_title,
				profile.getName()));
	...
	public void create() {
		...
		if (!fNewProfile) {
			...
			((IModifyDialogTabPage) fTabFolder.getSelection()[0].getData())
					.setInitialFocus();
		}
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		fProfileNameField
				.setLabelText(FormatterMessages.ModifyDialog_ProfileName_Label);
	...
	private void applyPressed() {
		if (!fProfile.getName().equals(fProfileNameField.getText())) {
			fProfile = fProfile.rename(fProfileNameField.getText(),
					fProfileManager);
		}
	...
	private void saveButtonPressed() {
		Profile selected = new CustomProfile(fProfileNameField.getText(),
				new HashMap(fWorkingValues), fProfile.getVersion(),
		...
		final String lastPath = PEXUIPlugin.getDefault()
				.getDialogSettings().get(fLastSaveLoadPathKey + ".savepath"); //$NON-NLS-1$
		if (lastPath != null) {
	...
	private void doValidate() {
		...
		if (name.equals(fProfile.getName())
				&& fProfile.hasEqualSettings(fWorkingValues,
						fWorkingValues.keySet())) {
		...
		if (!name.equals(fProfile.getName())
				&& fProfileManager.containsName(name)) {
			updateStatus(new Status(IStatus.ERROR, PEXUIPlugin.PLUGIN_ID,
					FormatterMessages.ModifyDialog_Duplicate_Status));
			return;
		}

		if (fProfile.isBuiltInProfile() || fProfile.isSharedProfile()) {
			updateStatus(new Status(IStatus.INFO, PEXUIPlugin.PLUGIN_ID,
	...
	private IStatus validateProfileName() {
		...
		if (fProfile.isBuiltInProfile()) {
			if (fProfile.getName().equals(name)) {
				return new Status(IStatus.ERROR, PEXUIPlugin.PLUGIN_ID,
